...
public class RootResource extends NamedCollectionResource<WebdavUser> implements MakeCollectionableResource, PutableResource {
	protected RootResource(WebdavUser wu) { 
		super(wu, wu.getUserId(), wu.getUsername()); 
	}
	
	@Override
	protected void ensureChildren() {
		Key<WebdavFolder> rootKey = ofy.query(WebdavFolder.class).filter("user", ofy.getFactory().getKey(_obj)).getKey();
		for(WebdavFolder wf : ofy.query(WebdavFolder.class).ancestor(rootKey))
